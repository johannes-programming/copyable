...
test: beautiful
	conda run -n base python make/env.py test_copyable --python=3.11 --recreate >/dev/null;
	conda run -n test_copyable pip install -e . >/dev/null;
	conda run -n test_copyable python -c 'import copyable.tests; copyable.tests.test();'
	conda run -n test_copyable pip install mypy >/dev/null;
	conda run -n test_copyable python -m mypy --exclude build --strict .;
	conda run -n test_copyable python -m mypy --strict -p copyable;
